...
Current legacy classification: normal room/message/attachment/thread/sync routes are `temporary fallback` when their Core cutover flag is off; their JSON responses include `messagingCoreCutover.source: "voyager_legacy"` plus `fallbackReason` and the same flag snapshot when they remain on the legacy backend. `/v1/realtime/token` is also a `temporary fallback` response while the client Core realtime opt-in remains guarded; `/v1/realtime` is the matching legacy WebSocket path and cannot add JSON diagnostics. Call lifecycle/media routes are frozen as `temporary call fallback` in Voyager while the Core call implementation remains config-gated and rollback stays available; call JSON responses include `messagingCoreCutover.source: "voyager_legacy"` with `fallbackReason: "call_cutover_not_implemented"`. Voyager admin, auth, account/device, sidebar, agent provisioning/review, and product usage/audit routes are `product-only`; the `/v1/messaging-core/*` validation proxies and readonly backfill route are `ready for removal` after the product no longer needs migration diagnostics.
