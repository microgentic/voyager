...
When `VOYAGER_MESSAGING_CORE_MESSAGE_CUTOVER=1` is set and the bridge can mint/sync a Messaging Core identity, normal Voyager message, thread, and attachment routes proxy to the matching Core public routes and include `messagingCoreCutover` metadata with the Core route and upstream status for JSON responses. The facade adapts Core-neutral message, receipt, thread-state, and attachment shapes back to existing Voyager client fields, including `protocolType`, `ciphertext`, Voyager receipt status names, and Voyager attachment variant/download metadata. This guarded slice covers message list/send, attachment-backed send/edit for Core-owned attachments, delete, ack, reactions, pins, forwarding, thread reads, standard thread replies, thread subscription, attachment allocation/upload/complete/download/delete, and authenticated private blob delivery. Thread inbox reads stay on Voyager until Core supports Voyager-compatible cursor pagination. Thread reply writes with Voyager's `alsoSendToRoom` duplicate-room-message semantics return `409 messaging_core_thread_broadcast_pending` until Core models that behavior. Existing Voyager-only visibility state from delete-for-me and existing Voyager-only attachment rows are not imported by the current readonly Core backfill, so this flag should only be used for Core-owned message/attachment write-cutover smoke or environments where existing hidden-message and legacy-attachment history are not being routed through the Core message path yet.
