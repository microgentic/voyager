...
`npm run smoke:messaging-core-parity` requires Core `/rooms` to return at least one room. The dev deployment now satisfies that through normal all-Core write/population flows rather than a Voyager readonly backfill route.
