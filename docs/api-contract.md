...
The web client can opt into the long-term Messaging Core realtime transport with `VITE_MESSAGING_CORE_REALTIME=1` at build time or `localStorage.setItem("voyager.messagingCoreRealtime", "1")` in a running client. When enabled, the client requests `/v1/messaging-core/realtime/token`, opens the returned Core `connectPath` on `messagingCore.baseUrl`, sends the `mrt_...` token as a query parameter, and uses the returned `messaging.realtime.v1` protocol. If Core token minting fails, or if the Core WebSocket closes before the `ready` frame, the client falls back to the existing Voyager `/v1/realtime/token` + `/v1/realtime` path.
...
Current legacy classification: normal room/message/attachment/thread/sync routes are `temporary fallback` when their Core cutover flag is off; `/v1/realtime/token` and `/v1/realtime` are `temporary fallback` while the client Core realtime opt-in remains guarded; Voyager admin, auth, account/device, sidebar, agent provisioning/review, and product usage/audit routes are `product-only`; the `/v1/messaging-core/*` validation proxies and readonly backfill route are `ready for removal` after the product no longer needs migration diagnostics.
