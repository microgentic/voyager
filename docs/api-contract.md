...
| `GET` | `/v1/devices/me/push-tokens` | `{ pushTokens }` |
| `POST` | `/v1/devices/me/push-tokens` | `{ pushToken }` |
| `DELETE` | `/v1/devices/me/push-tokens/{pushTokenId}` | `{ ok: true }` |
| `POST` | `/v1/devices/{deviceId}/revoke` | `{ ok: true }` |

Login accepts `email`, `password`, and an optional `device` object. Existing-device login may send `device.deviceId`. Today that is a development contract; future device private-key proof can extend this without changing the route. Auth and session-restore responses include the exact current `session` object so clients can distinguish the current login token from other sessions on the same device. `GET /v1/sessions` returns active, non-expired sessions only. `GET /v1/devices` returns active, non-revoked devices only. If password login is valid but the supplied `device.deviceId` is missing or revoked, Voyager returns `403 device_not_available` with `details.reason: "device_revoked_or_missing"`, `details.canReenroll: true`, `details.clearStoredDeviceId: true`, and `details.retryWithoutDeviceId: true`; clients should clear the stored device id and let the next explicit sign-in register a fresh device.

Native push registration is current-device scoped. After iOS receives an APNs device token, it should call `POST /v1/devices/me/push-tokens` with `{ "provider": "apns", "environment": "development" | "production", "bundleId": "...", "token": "<hex APNs token>" }`. The response intentionally returns token metadata but never the raw token or token hash. `GET /v1/devices/me/push-tokens` lists active tokens for the current device, and `DELETE /v1/devices/me/push-tokens/{pushTokenId}` disables a token on logout or APNs token replacement. Registering a token marks the current device `notificationCapability: "apns"`; unregistering the last active token clears it. Logout, session revocation, and device revocation disable matching token rows, and notification dispatch also requires an active account, active principal, active device, and active non-expired session.

APNs delivery is best-effort and never blocks message/call/invitation writes. Voyager sends privacy-safe alert payloads only: `"New message"`, `"New reply"`, `"Incoming call"`, or `"New room invitation"`, plus routing identifiers such as `type`, `roomId`, `envelopeId`, `rootEnvelopeId`, `callId`, or `invitationId`. The backend does not put plaintext message content in APNs payloads. Production APNs sending requires `APNS_TEAM_ID`, `APNS_KEY_ID`, `APNS_PRIVATE_KEY`, and either `APNS_BUNDLE_ID` or `APNS_ALLOWED_BUNDLE_IDS`; the bundle setting restricts accepted token topics. The dev Worker config allowlists the native iOS bundle ID `com.microgentic.voyager.ios`, and the deploy workflow syncs APNs auth secrets when GitHub Actions secrets are configured. If APNs is not configured, token registration still works but delivery is skipped with a structured Worker warning. APNs `BadDeviceToken`, `DeviceTokenNotForTopic`, and `Unregistered` responses invalidate stored token rows.
...
Normal Voyager message, thread, and attachment routes proxy to the matching Core public routes and include `messagingCoreCutover` diagnostics for JSON responses. The facade adapts Core-neutral message, receipt, thread-state, thread-inbox, and attachment shapes back to existing Voyager client fields, including `protocolType`, `ciphertext`, Voyager receipt status names, room-enriched thread inbox items, `alsoSentToRoom` for thread replies that also appear in the room timeline, and Voyager attachment variant/download metadata. This covers message list/send, attachment-backed send/edit for Core-owned attachments, delete-for-me visibility, delete-for-everyone tombstones, ack, reactions, pins, forwarding, thread reads, standard thread replies, `alsoSendToRoom` thread replies, thread inbox reads, thread subscription, attachment allocation/upload/complete/download/delete, and authenticated private blob delivery.
...
Rollback for messaging is operational, not a runtime switch: redeploy a known-good rollback tag/branch or restore the exported pre-removal D1/R2 backup. This repository must not reintroduce a live Voyager-owned rooms/messages/attachments/threads/sync implementation as a hidden fallback.

For a configured Voyager API and Messaging Core service, the parity smoke verifies that Voyager exposes a Messaging Core session through `/v1/me`, that Core accepts it for `/me`, `/bootstrap`, `/sync`, `/realtime/token`, `/rooms`, and related read paths, and that the normal Voyager room/message/thread/attachment/sync routes report `messagingCoreCutover.source: "core"`. It also opens the Core WebSocket returned by the Voyager realtime-token facade, verifies `ready` plus `pong` over `messaging.realtime.v1`, sends messages through the normal Voyager message route, verifies Core realtime delivery, exercises thread inbox and `alsoSendToRoom`, and allocates/uploads/completes/references/lists/downloads a Core-owned attachment through normal Voyager routes. The smoke fails if any JSON response contains `source: "voyager_legacy"`.
