...
When `VOYAGER_MESSAGING_CORE_MESSAGE_CUTOVER=1` is set and the bridge can mint/sync a Messaging Core identity, normal Voyager message, thread, and attachment routes proxy to the matching Core public routes and include `messagingCoreCutover` diagnostics for JSON responses. The facade adapts Core-neutral message, receipt, thread-state, thread-inbox, and attachment shapes back to existing Voyager client fields, including `protocolType`, `ciphertext`, Voyager receipt status names, room-enriched thread inbox items, `alsoSentToRoom` for thread replies that also appear in the room timeline, and Voyager attachment variant/download metadata. This guarded slice covers message list/send, attachment-backed send/edit for Core-owned attachments, delete-for-me visibility, delete-for-everyone tombstones, ack, reactions, pins, forwarding, thread reads, standard thread replies, `alsoSendToRoom` thread replies, thread inbox reads, thread subscription, attachment allocation/upload/complete/download/delete, and authenticated private blob delivery.
