...
| `GET` | `/v1/messaging-core/sync` | `{ messagingCore, rooms, roomsNextCursor, pendingMessages, serverTime, proxied }` |
...
The `/v1/messaging-core/*` proxy routes are cutover validation routes. They authenticate the caller with the Voyager session, mint/sync a Messaging Core session, call the matching product-neutral Core route, and return Core's payload plus proxy metadata. Current proxy coverage includes Core `/bootstrap`, `/sync`, `/realtime/token`, `/rooms`, `/rooms/{roomId}`, and `/rooms/{roomId}/messages`. The realtime-token validation route intentionally returns Core's `mrt_...` token under `realtime` and does not replace Voyager's live `/v1/realtime/token` response unless the web client explicitly opts into Core realtime.

The web client can opt into the long-term Messaging Core realtime transport with `VITE_MESSAGING_CORE_REALTIME=1`, with the all-Core app-mode alias `VITE_MESSAGING_CORE_ALL_CUTOVER=1`, or with `localStorage.setItem("voyager.messagingCoreRealtime", "1")` in a running client. Local storage can also force realtime fallback with `"0"` or `"false"` for rollback. When enabled, the client requests `/v1/messaging-core/realtime/token`, opens the returned Core `connectPath` on `messagingCore.baseUrl`, sends the `mrt_...` token as a query parameter, and uses the returned `messaging.realtime.v1` protocol. If Core token minting fails, or if the Core WebSocket closes before the `ready` frame, the client falls back to the existing Voyager `/v1/realtime/token` + `/v1/realtime` path.

When `VOYAGER_MESSAGING_CORE_ROOM_CUTOVER=1` is set and the bridge can mint/sync a Messaging Core identity, normal Voyager room routes proxy to the matching Core public room routes and return Voyager-compatible response envelopes. The facade adapts Core-neutral room, member, invitation, and ownership-transfer shapes back to existing Voyager client fields, including account/principal display enrichment from Voyager identity rows. JSON cutover responses include `messagingCoreCutover` diagnostics with `source: "core"`, `fallbackReason: null`, the Core route, upstream status, and a sanitized flag snapshot. This guarded slice covers room list/detail/create/update/archive, membership add/role/remove/leave, pending invitation list/create/respond, and ownership transfer propose/accept. Non-pending room-invitation list filters remain on the Voyager path until Core exposes historical invitation pagination.

When `VOYAGER_MESSAGING_CORE_MESSAGE_CUTOVER=1` is set and the bridge can mint/sync a Messaging Core identity, normal Voyager message, thread, and attachment routes proxy to the matching Core public routes and include `messagingCoreCutover` diagnostics for JSON responses. The facade adapts Core-neutral message, receipt, thread-state, thread-inbox, and attachment shapes back to existing Voyager client fields, including `protocolType`, `ciphertext`, Voyager receipt status names, room-enriched thread inbox items, and Voyager attachment variant/download metadata. This guarded slice covers message list/send, attachment-backed send/edit for Core-owned attachments, delete, ack, reactions, pins, forwarding, thread reads, standard thread replies, thread inbox reads, thread subscription, attachment allocation/upload/complete/download/delete, and authenticated private blob delivery. Thread reply writes with Voyager's `alsoSendToRoom` duplicate-room-message semantics return `409 messaging_core_thread_broadcast_pending` until Core models that behavior. Existing Voyager-only visibility state from delete-for-me and existing Voyager-only attachment rows are not imported by the current readonly Core backfill, so this flag should only be used for Core-owned message/attachment write-cutover smoke or environments where existing hidden-message and legacy-attachment history are not being routed through the Core message path yet.

When `VOYAGER_MESSAGING_CORE_SYNC_CUTOVER=1` is set, normal `GET /v1/sync` calls Core `/sync`, expands Core room summaries through Core room detail, adapts rooms and pending messages back to Voyager-compatible shapes, and returns the existing `{ ok, sync }` envelope plus `messagingCoreCutover` diagnostics. `VOYAGER_MESSAGING_CORE_ALL_CUTOVER=1` enables the room, message/attachment/thread, and sync cutover flags together for dev/smoke proof that Voyager can run its messaging traffic through Core while legacy fallback remains available by turning the flag off. The dev Worker deployment workflow now passes this composite flag through `npm run deploy:dev`; rollback is a deploy/config flip back to `0`.

Current legacy classification: normal app-bootstrap, messaging identity/key-package, room/message/attachment/thread/sync routes are `temporary fallback` when their Core cutover flag is off or no cutover flag exists yet; their JSON responses include `messagingCoreCutover.source: "voyager_legacy"` plus `fallbackReason` and the same flag snapshot when they remain on the legacy backend. `/v1/realtime/token` is also a `temporary fallback` response while the client Core realtime opt-in remains guarded; `/v1/realtime` is the matching legacy WebSocket path and cannot add JSON diagnostics. Call lifecycle/media routes are frozen as `temporary call fallback` in Voyager while the Core call implementation remains config-gated and rollback stays available; call JSON responses include `messagingCoreCutover.source: "voyager_legacy"` with `fallbackReason: "call_cutover_not_implemented"`. Voyager admin, auth, account/device-management, sidebar, agent provisioning/review, and product usage/audit routes are `product-only`; the `/v1/messaging-core/*` validation proxies and readonly backfill route are `ready for removal` after the product no longer needs migration diagnostics.

The executable boundary inventory lives in `messagingCoreBoundaryCatalog` in `scripts/api-contract-assertions.mjs`. `node scripts/route-inventory-check.mjs` validates that every Core-owned Voyager route in the endpoint catalog is classified as `temporary-fallback`, `temporary-call-fallback`, or `ready-for-removal`, which keeps new legacy ownership from appearing silently during the cutover period.
...
For a configured Voyager API and Messaging Core service, the optional parity smoke verifies that Voyager can mint a Messaging Core token, that Core accepts it for `/me`, `/bootstrap`, `/sync`, `/realtime/token`, `/rooms`, and related read paths, and that the Voyager validation proxies match direct Core identity and room/message/sync/realtime-token reads. Set `SMOKE_MESSAGING_CORE_REALTIME_CONNECT=1` to additionally open the Core WebSocket returned by the Voyager realtime-token proxy and verify `ready` plus `pong` over `messaging.realtime.v1`. Set `SMOKE_MESSAGING_CORE_WRITE_CUTOVER=1` only when the deployed Voyager Worker has `VOYAGER_MESSAGING_CORE_MESSAGE_CUTOVER=1`; the smoke then sends a plain message through the normal `/v1/rooms/{roomId}/messages` route and verifies that the normal route was proxied to Core. It also creates a standard thread reply, verifies the normal Voyager thread inbox facade, and allocates, uploads, completes, references, lists, and downloads a Core-owned attachment through normal Voyager attachment/message routes. For normal dev deployments, set `SMOKE_MESSAGING_CORE_ALL_CUTOVER=1`; the smoke expects the deployed Voyager Worker to have `VOYAGER_MESSAGING_CORE_ALL_CUTOVER=1`, enables room, room-write, message/thread/attachment, sync, and Core realtime checks together, then proves normal Voyager sync and Core realtime message delivery for a message sent through the normal Voyager message route.
