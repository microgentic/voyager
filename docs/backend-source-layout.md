...
- Any new, removed, or changed Worker route must update
  ...
  same PR. The route inventory guard scans `src/index.ts`, `src/backend/routes.ts`,
  and the route-group modules under `src/backend/routing/`; it depends on the
  catalog to catch dropped handlers and undocumented `/v1` routes.
