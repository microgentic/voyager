...
Password/passphrase authentication is active. Clients should persist `device.deviceId` and send it on password login to reuse an enrolled device; omitting it enrolls a new device. Passkeys/WebAuthn, push providers, app-store packaging, code signing, live agent runtimes, Queues, external identity providers, and provider webhooks are intentionally deferred for the backend-first path. Foreground Durable Object WebSocket event hints were added after the original backend-first pass; Conversation Durable Objects now coordinate message sends and room/membership mutations, while deeper D1/DO reconciliation remains future work.
